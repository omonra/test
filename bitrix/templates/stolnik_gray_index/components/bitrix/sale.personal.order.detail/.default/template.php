				...
				*/?>
				...
				<tr>
					...
					<td>
						...
						<?else:?>
							<?// =GetMessage('SPOD_NO')?>
							<?if ($arResult['STATUS_ID'] == 'O'):?>
								<?if($arResult["CAN_REPAY"]=="Y" && $arResult["PAY_SYSTEM"]["PSA_NEW_WINDOW"] == "Y"):?>
									<?if ($arResult['PAY_SYSTEM_ID'] == 14):?>
										<a href="<?=$arResult["PAY_SYSTEM"]["PSA_ACTION_FILE"]?>" class="bx_big bx_bt_button_type_2 bx_cart bx_order_action">
											Оплатить
										</a>
									<br/>
									<?else:?>
										<span id="jsloadPaysystem"></span>
										<script>
											$('#jsloadPaysystem').load('<?=$arResult["PAY_SYSTEM"]["PSA_ACTION_FILE"]?>',function(){
												$('#jsloadPaysystem input[type=submit]').addClass('bx_big bx_bt_button_type_2 bx_cart bx_order_action');
												$('#jsloadPaysystem input[type=submit]').css('min-width', '140px');
											});
										</script>
									<?endif;?>
