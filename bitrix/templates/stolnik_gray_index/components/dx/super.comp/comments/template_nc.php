...
<div class="body g-clear" id="comments-block">
    <div class="b-reviews">
        <?foreach ($arResult['ITEMS'] as $arItem):?>
            <p>
                <strong><?=$arItem['PROPERTY_NAME_VALUE']?></strong>
    ...
    <form class="b-reviews-form" action="#reviews_form" method="post">
        ...
        <input type="text" id="comments-field-name" name="name" value="<?=(strlen($arResult['VARS']['name']) > 0 ? $arResult['VARS']['name'] : '')?>"/>

        <label for="comments-field-email">Электронная почта</label>
        <input type="text" id="comments-field-email" name="email" value="<?=(strlen($arResult['VARS']['email']) > 0 ? $arResult['VARS']['email'] : '')?>"/>
