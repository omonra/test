...
<input id="ORDER_PROP_25" class="text_input text_input_1" type="hidden" value="<?if(!empty($arUser['UF_BLACK']))echo "ГѓВў ГѓВ·Г‚ВёГѓВ°ГѓВ­ГѓВ®ГѓВ¬ ГѓВ±ГѓВЇГѓВЁГѓВ±ГѓВЄГѓВҐ";?>" name="ORDER_PROP_25">
<input type="hidden" class="hidden_city" name="ORDER_PROP_5" value="161">

<input id="ORDER_PROP_4" type="hidden" name="ORDER_PROP_4" value="454000">
<input id="ORDER_PROP_20"  type="hidden" name="ORDER_PROP_20" value="Не указано(быстрый заказ)">
<input id="ORDER_PROP_21"  type="hidden" name="ORDER_PROP_21" value="Не указано(быстрый заказ)">
...
function PrintPropsForm($arSource=Array())
{
	if (!empty($arSource))
	{
		...
		foreach($arSource as $arProperties)
		{
			...
			?>
            <dt <?if($arProperties['CODE'] == 'LOCATION'):?>style="display:none;"<?endif;?>>
            <?
            ...
            <label for="<?=$arProperties["FIELD_NAME"]?>">
            	<?if ($arProperties["ID"] == 7){
 						$arProperties["NAME"] = "Адрес";           		 	
            	}?>
            	<?= $arProperties["NAME"] ?><?if (strlen($arProperties["DESCRIPTION"]) > 0):?><span class="description">(<?=$arProperties["DESCRIPTION"]?>)</span><?endif;?></label><?if($arProperties["REQUIED_FORMATED"]=="Y"):?><span class="footnote">*</span><?endif;?></dt>
                    ...
                    <script type="text/javascript">
                            ...
                            <?foreach($arSource as $arPropCity)
                        {
                            ...
                            $(".input_autocomplete").focusout(function(){
                                // ГѓВ±ГѓВўГѓВҐГѓВ°ГѓВїГѓВҐГѓВ¬ ГѓВ±ГѓВ¬ГѓВҐГѓВ­ГѓВЁГѓВ«ГѓВ®ГѓВ±ГѓВј ГѓВ«ГѓВЁ ГѓВ§ГѓВ­ГѓВ ГѓВ·ГѓВҐГѓВ­ГѓВЁГѓВҐ
                            ...
                            function handleCityChange(input)
                            {
                                ...
                                if(CITY_VAL != $(".input_autocomplete").val() )
                                {
                                    ...
                                    // ГѓВ­ГѓВҐ ГѓВ­ГѓВ ГѓВёГѓВ«ГѓВЁ ГѓВ±ГѓВ®ГѓВўГѓВЇГѓВ ГѓВ¤ГѓВҐГѓВ­ГѓВЁГѓВї, ГѓВўГѓВ»ГѓВЎГѓВЁГѓВ°ГѓВ ГѓВҐГѓВ¬ ГѓВЇГѓВіГѓВ±ГѓВІГѓВ®ГѓВ© ГѓВЈГѓВ®ГѓВ°ГѓВ®ГѓВ¤
                                    for ( keyVar in CITIES ) {
                                        if( '' == CITIES[keyVar] )
                                        {
                                            $('.hidden_city').val(keyVar);
...
<div class="coments">
        <h2>Дополнительные комментарии к заказу</h2>
        
    <textarea style="width:99%" rows="5" name="ORDER_DESCRIPTION"></textarea></div>

    <input  id="but_of_zak" style="color: #F55;" class="button button_1 submit" type="button" name="submitbutton" onclick="quickFormSubmit()" value="Оформить заказ">