...
var continue_flag;
/*
var msg = "Пожалуйста полностью заполните поле Ф.И.О. по шаблону (Иванов Иван Иванович).";
...
if(!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "N")
{
    if(!empty($arResult["ERROR"]))
    {
		...
		echo "<a href=\"/login/\">Авторизуйтесь</a>, чтобы оформить заказ, или введите иной e-mail.<br />";
    }
    ...
    else
    {
        ...
        if(!empty($arResult["ERROR"]) && $arResult["USER_VALS"]["FINAL_STEP"] == "Y")
        {
            foreach($arResult["ERROR"] as $v)
                echo ShowError($v);
			if($arResult["ERROR"]['reg_error'])
		echo "<a href=\"/login/\" style=\"color:red;\">Авторизуйтесь</a>, чтобы оформить заказ, или введите иной e-mail.<br /><br />";
            ?>
            <script>
                ...
                <div id="order_form_id">
					<div class="ordering_sections">
						...
						<div class="ordering_section ordering_section_icos">
							...
							<div class="ordering_section_ico"><p style="padding-top:5px;">Мы гарантируем безопасность покупки.</p></div>
							<div class="ordering_section_ico"><img src="/bitrix/templates/stolnik_gray/images/basket/delivery.png"></div>
							<div class="ordering_section_ico"><p>Доставка по России бесплатно.</p></div>
							<div class="ordering_section_ico"><img src="/bitrix/templates/stolnik_gray/images/basket/hanger.png"></div>
							<div class="ordering_section_ico"><p style="padding-top:12px;">Примерка перед покупкой</p></div>
						</div>
					</div>
					<? //echo $arResult["disc"];//global $USER; if ($USER->IsAdmin()) echo "<pre>"; print_r($arResult); echo "</pre>";
					?>
<? if(0 > 1) { ?>
					<p style="color:red;">При заказе товара на сумму свыше 1000 руб доставка по России бесплатно. </p>
<? } ?>
...
<? if(0 > 1) { // скрываем стоимость доставки... но как быть с "не по России" ?>
						<?if($arResult["DELIVERY_PRICE"]>0):?><dl class="summ"><dt>Доставка:</dt><dd><span class="val"><?=substr($arResult["DELIVERY_PRICE_FORMATED"],0,strlen($arResult["DELIVERY_PRICE_FORMATED"])-4)?></span> руб.</dd></dl><br /><?endif;?>
<? } ?>
						<?
							$discount=0;
						...
						<?/*if($discount > 0){?>
							...
							<?echo "<p class='disc_val'>Скидка: ".$arResult["BASKET_ITEMS"][0]['DISCOUNT_PRICE_PERCENT_FORMATED']."  (".$discount." руб.)</p>";?>
							$arResult["ORDER_TOTAL_PRICE_FORMATED"]
						<?}*/?>

						<dl class="summ"><dt>Итого:</dt><dd><span class="val"><?=substr($arResult["ORDER_PRICE_FORMATED"],0,strlen($arResult["ORDER_PRICE_FORMATED"])-4)?></span> руб.</dd></dl>

						<input id="but_of_zak" style="color: #F55;" class="button button_1 submit" type="button" name="submitbutton"  onclick="submitForm('Y')" value="Оформить заказ">
						<dl style="float:right;margin: 0px 20px 0 0px;"><!--input type="checkbox" id="yes_dogowor" onchange="if($(this).attr('checked')=='checked') { $('#but_of_zak').removeAttr('disabled'); $('#but_of_zak').css('color','#FFF')}else{$('#but_of_zak').attr('disabled','disabled'); $('#but_of_zak').css('color','#F55')}"--> Нажимая на кнопку "Оформить заказ", вы принимаете условия <a href="/articles/publichnaya-ofera/">публичной оферты</a></dl>
					</div>


					<?//echo "<p class='disc_info'>При заказе товара на сумму свыше 10000 руб предоставляется скидка в размере 5% на сумму всего заказа.</p>";?>
					<?//echo "<p class='disc_info2'>При заказа товара на сумму свыше 1000 руб доставка по России бесплатно. По Челябинску бесплатная доставка при заказе на сумму свыше 500 руб.</p>";?>
