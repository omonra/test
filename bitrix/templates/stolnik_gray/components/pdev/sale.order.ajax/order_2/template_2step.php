    	...
    	if(continue_flag == false && val=='Y'){
	        ...
	        var options = {
	            ...
	            success: function(data, textStatus, jqXHR) {
	                ...
	                if(!continue_flag){$('#ORDER_PROP_1').css('border', "1px solid red");}
	            }
	        };
    ...
    else
    {
        ...
        <form id="ORDER_FORM_ID_NEW" name="<?=$FORM_NAME?>" method='post' action=''>
            ...
            <div class="orders_block">
                ...
                <div id="order_form_id">
					<ul class="order_tabs">
						<li class="order_back active"><a href="#order_contacts">Contacts</a></li>
						<li class="no_bg">--></li>
						<li class="order_next"><a href="#order_delivery">Delivery</a></li>
					</ul>
					<div id="order_contacts" class="order_block order_block_active">
						&nbsp;
						<?
						...
						?>
						<a href="#order_delivery" id="order_next" class="button button_1">NEXT</a>
					</div>
		...
		<?endif?>
						...
						<div class="ordering_options">
							<?if($arResult["DELIVERY_PRICE"]>0):?><dl class="summ" style="margin: -14px 0 -12px 0px;"><dt>Доставка:</dt><dd><span class="val"><?=substr($arResult["DELIVERY_PRICE_FORMATED"],0,strlen($arResult["DELIVERY_PRICE_FORMATED"])-4)?></span> руб.</dd></dl><br /><?endif;?>
							<?
								$discount=0;
							...
							<dl class="summ"><dt>Итого:</dt><dd><span class="val"><?=substr($arResult["ORDER_TOTAL_PRICE_FORMATED"],0,strlen($arResult["ORDER_TOTAL_PRICE_FORMATED"])-4)?></span> руб.</dd></dl>
							<a href="#order_contacts" id="order_back" class="button button_1">BACK</a>
							<input disabled="disabled" id="but_of_zak" style="color: #F55;" class="button button_1 submit" type="button" name="submitbutton"  onclick="submitForm('Y')" value="Оформить заказ">
							<dl style="float:right;margin: 0px 20px 0 0px;"><input type="checkbox" id="yes_dogowor" onchange="if($(this).attr('checked')=='checked') { $('#but_of_zak').removeAttr('disabled'); $('#but_of_zak').css('color','#FFF')}else{$('#but_of_zak').attr('disabled','disabled'); $('#but_of_zak').css('color','#F55')}"> Согласен с условиями <a href="/articles/publichnaya-ofera/">публичной оферты</a></dl>
						</div>


						<?//echo "<p class='disc_info'>При заказе товара на сумму свыше 5000 руб предоставляется скидка в размере 5% на сумму всего заказа.</p>";?>
						<?//echo "<p class='disc_info2'>При заказа товара на сумму свыше 3000 руб доставка по России бесплатно. По Челябинску бесплатная доставка при заказе на сумму свыше 500 руб.</p>";?>
					</div>
