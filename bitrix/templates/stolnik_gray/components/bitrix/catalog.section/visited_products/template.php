...
<div class="b-product-card__recomended recomend-two">
    <div class="b-title">
        Рекомендуем
    </div>
...
<div class="b-carousel-novelty">
    <div class="b-title">Вы недавно смотрели</div>
    <ul class="b-list b-list_products b-list_products_four g-clear">
        <?foreach($arResult["ITEMS"] as $key => $arItem):
            $this->AddEditAction($arItem['ID'], $arItem['EDIT_LINK'], CIBlock::GetArrayByID($arParams["IBLOCK_ID"], "ELEMENT_EDIT"));
            $this->AddDeleteAction($arItem['ID'], $arItem['DELETE_LINK'], CIBlock::GetArrayByID($arParams["IBLOCK_ID"], "ELEMENT_DELETE"), array("CONFIRM" => 'Будет удалена вся информация, связанная с этой записью. Продолжить?'));
            ?>
